...
function initValidation(formName) {
  ...
  $(':input').change(function(ev){
    ...
    if(this.name =='find-page'){
      if(!document.getElementById('newspaper').checkValidity())
      {
        $('#newspaper').addClass("was-validated");
      }
    }
...
function validateState(id, msg){
  ...
  valid = stateAbbreviations.indexOf($el[0].value.toUpperCase()) > -1;
...
function validateCheckboxGroup(fieldName, message) {
  ...
  for( let theItem of document.getElementsByName($(fieldName)[0].name) ){
    if(theItem.checked){
      valid = true;
    }
  }
